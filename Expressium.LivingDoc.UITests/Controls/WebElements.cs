...
namespace Expressium.LivingDoc.UITests.Controls
{
    public static class WebElements
    {
        ...
        public static bool IsActive(this By locator, IWebDriver driver)
        {
            var attribute = locator.WaitForElementIsVisible(driver).GetAttribute("class");
            if (!string.IsNullOrEmpty(attribute) && attribute.Contains("active"))
                return true;
